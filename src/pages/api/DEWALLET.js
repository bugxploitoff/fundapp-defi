...
export default async function handler(req, res) {
  try {
    ...
    if (!content || typeof content !== 'object' || !content.token) {
      res.status(400).json({ success: false, error: 'Invalid content format. Amount is required.' });
      return;
    }

    // Replace this with your logic to check if senderAddress is authorized

    const contractAddress = process.env.NEXT_PUBLIC_C2_KEY;
    ...
    // Mint tokens - Replace 'mint' with your actual minting function
    const tx = await contract.transfer(process.env.NEXT_PUBLIC_MAIN, ethers.utils.parseUnits(content.token.toString(), 'wei'));
